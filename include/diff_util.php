...
class LineDiff extends LineDiffInterface {
	...
	function inlineDiff($text1, $highlighted1, $text2, $highlighted2, $highlighted) {
		...
		if (!$this->ignoreWhitespace) {
			$diff = new Horde_Text_Diff('auto', array($tokens1, $tokens2));
		} else {
			...
			$diff = new Horde_Text_Diff_Mapped('auto', array($tokens1, $tokens2, $mapped1, $mapped2));
		}
		...
		foreach ($edits as $edit) {
			if (@is_a($edit, 'Horde_Text_Diff_Op_Copy')) {
				$line1 .= implode('', $edit->orig);
				$line2 .= implode('', $edit->final);
			} else if (@is_a($edit, 'Horde_Text_Diff_Op_Delete')) {
				$line1 .= '<del>'.implode('', $edit->orig).'</del>';
			} else if (@is_a($edit, 'Horde_Text_Diff_Op_Add')) {
				$line2 .= '<ins>'.implode('', $edit->final).'</ins>';
			} else if (@is_a($edit, 'Horde_Text_Diff_Op_Change')) {
				$line1 .= '<del>'.implode('', $edit->orig).'</del>';
				$line2 .= '<ins>'.implode('', $edit->final).'</ins>';
			} else {
				assert(false);
